...
const ItemsManager = () => {
  ...
  const handleSubmit = async e => {
    ...
    //   sauces: form.sauceGroupId
    // ? doc(db, 'sauceGroups', form.sauceGroupId)
    // : null,
    };
    ...
    try {
      if (editId) {
        ...
        // Get current count to generate new ID
         const snapshot = await getDocs(collection(db, 'items'));
         const newId = `item${snapshot.size + 1}`;
        await setDoc(doc(db, 'items',newId), itemData);
      }

      setForm({ itemName: '', price: '', categoryId: '', saucesArray: [] });
  ...
  const handleEdit = item => {
    setForm({
      ...
      saucesArray: Array.isArray(item.sauces)
        ? item.sauces.map(s => s.id)
        : [],
    });
    setEditId(item.id);
  };
  ...
  const handleSauceSelect = e => {
    ...
    setForm({ ...form, saucesArray: selectedOptions });
  };
        ...
        <table className="w-full border border-gray-300">
          ...
          <tbody>
            {items.length === 0 ? (
              ...
              items.map(item => {
                const categoryName =
                  categories.find(cat => cat.id === item.categoryId?.id)
                    ?.name || item.categoryId?.id || '—';
